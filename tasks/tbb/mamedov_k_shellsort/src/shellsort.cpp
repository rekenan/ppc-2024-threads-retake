...
std::vector<int> mamedov_k_shellsort_tbb::getRandomVec(int n) {
...
bool mamedov_k_shellsort_tbb::checkOrder(const std::vector<int>& vec) {
...
void mamedov_k_shellsort_tbb::shellsortSimpleMerge(std::vector<int>* vec, int parts, bool parallel) {
  ...
  if (parallel) {
    ...
    for (int i = 0; i < parts; i++) {
      group.run([&vecs, i]() { shellSort(&vecs[i]); });
    }
    group.wait();
  } else {
    for (auto& elem : vecs) {
...
bool mamedov_k_shellsort_tbb::ShellsortTBBTaskSequential::pre_processing() {
  internal_order_test();
...
bool mamedov_k_shellsort_tbb::ShellsortTBBTaskSequential::validation() {
  internal_order_test();
  return taskData->inputs_count[0] == taskData->outputs_count[0];
}

bool mamedov_k_shellsort_tbb::ShellsortTBBTaskSequential::run() {
  internal_order_test();
  shellsortSimpleMerge(&input_);
  return true;
}

bool mamedov_k_shellsort_tbb::ShellsortTBBTaskSequential::post_processing() {
  internal_order_test();
  std::copy(input_.begin(), input_.end(), reinterpret_cast<int*>(taskData->outputs[0]));
  return true;
}

bool mamedov_k_shellsort_tbb::ShellsortTBBTaskParallel::pre_processing() {
  internal_order_test();
  // Init value for input and output
...
bool mamedov_k_shellsort_tbb::ShellsortTBBTaskParallel::validation() {
  internal_order_test();
  return taskData->inputs_count[0] == taskData->outputs_count[0];
}

bool mamedov_k_shellsort_tbb::ShellsortTBBTaskParallel::run() {
  internal_order_test();
  shellsortSimpleMerge(&input_, std::thread::hardware_concurrency(), true);
  return true;
}

bool mamedov_k_shellsort_tbb::ShellsortTBBTaskParallel::post_processing() {
  internal_order_test();
  std::copy(input_.begin(), input_.end(), reinterpret_cast<int*>(taskData->outputs[0]));
  return true;
}
