...
std::vector<int> mamedov_k_shellsort_seq::getRandomVec(int n) {
...
bool mamedov_k_shellsort_seq::checkOrder(const std::vector<int>& vec) {
...
void mamedov_k_shellsort_seq::shellsortSimpleMerge(std::vector<int>* vec, int parts) {
...
bool mamedov_k_shellsort_seq::ShellsortSequential::pre_processing() {
  internal_order_test();
...
bool mamedov_k_shellsort_seq::ShellsortSequential::validation() {
  internal_order_test();
  return taskData->inputs_count[0] == taskData->outputs_count[0];
}

bool mamedov_k_shellsort_seq::ShellsortSequential::run() {
  internal_order_test();
  shellsortSimpleMerge(&input_);
  return true;
}

bool mamedov_k_shellsort_seq::ShellsortSequential::post_processing() {
  internal_order_test();
